...
class _NiagaraSceneHeader(ctypes.Structure):
    _fields_ = [
        ...
        ("omm_array_data_size", ctypes.c_uint32), ("omm_index_data_size", ctypes.c_uint32),
        ("omm_desc_count", ctypes.c_uint32), ("omm_states", ctypes.c_uint32),
        ("camera", _NiagaraCamera), ("sun_direction", ctypes.c_float * 3),
    ]
...
def main():
    ...
    scene_dir = Path(r"D:\niagara_bistro")
    position, target, up, fov, z_near, sun = load_niagara_inputs(scene_dir)
    ...
    scene.set_camera(position=position, target=target, up=up, fov_degrees=fov,
                     width=1024, height=768, z_near=z_near, z_far=200.0)
    scene.set_default_light(direction=[-v for v in sun], color=[1, 1, 1], irradiance=50.0)
    scene.set_ambient(top_rgb=[0.03, 0.03, 0.03], bottom_rgb=[0.01, 0.01, 0.01])

    out = Path(r"D:\RTXNS\output\bistro_test")
    out.mkdir(parents=True, exist_ok=True)

    # --- OMM OFF (fresh scene, builds BLAS without OMM) ---
    scene.enable_rt_shadows(True)
    scene.enable_omm(False)
    print("\nRendering OMM OFF (BLAS without OMM)...")
    img_off = scene.render_frame()
    s_off = scene.get_last_frame_stats()
    arr_off = np.frombuffer(img_off, np.uint8).reshape(768, 1024, 4)[:, :, :3]
    Image.fromarray(arr_off, "RGB").save(out / "omm_off.png")
    print(f"  OMM OFF: total={s_off['total_ms']:.1f}ms  shadow_ray={s_off['shadow_ray_ms']:.1f}ms  blas={s_off['blas_build_ms']:.1f}ms")

    # --- OMM ON (fresh scene, builds BLAS WITH OMM) ---
    rr.destroy()
    rr.init(runtime_dir=str(repo_root), backend="vulkan", device_index=-1, enable_debug=False)
    scene2 = rr.create_scene()
    scene2.load_scene(str(scene_dir / "bistro.gltf"))
    scene2.set_camera(position=position, target=target, up=up, fov_degrees=fov,
                      width=1024, height=768, z_near=z_near, z_far=200.0)
    scene2.set_default_light(direction=[-v for v in sun], color=[1, 1, 1], irradiance=50.0)
    scene2.set_ambient(top_rgb=[0.03, 0.03, 0.03], bottom_rgb=[0.01, 0.01, 0.01])
    scene2.enable_rt_shadows(True)
    scene2.enable_omm(True)

    print("\nRendering OMM ON (fresh scene, BLAS with OMM)...")
    img_on = scene2.render_frame()
    s_on = scene2.get_last_frame_stats()
    arr_on = np.frombuffer(img_on, np.uint8).reshape(768, 1024, 4)[:, :, :3]
    Image.fromarray(arr_on, "RGB").save(out / "omm_on.png")
    print(f"  OMM ON:  total={s_on['total_ms']:.1f}ms  shadow_ray={s_on['shadow_ray_ms']:.1f}ms  blas={s_on['blas_build_ms']:.1f}ms")

    # --- Pixel diff ---
    diff = np.abs(arr_on.astype(float) - arr_off.astype(float)).max(axis=2)
    print(f"\nOMM ON/OFF pixel diff: max={diff.max():.0f}  mean={diff.mean():.2f}")

    # Save diff image
    diff_vis = np.clip(diff * 20, 0, 255).astype(np.uint8)
    Image.fromarray(diff_vis, "L").save(out / "omm_diff.png")
